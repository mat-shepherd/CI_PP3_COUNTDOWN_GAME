...
One test failed for validate_user_numbers function, which higlighted the need to check for a string that contains just spaces. This was corrected using the  isspace() method.
...
| Input trailing and leading spaces [Fixed] | Trailing and leading spaces on inputs wasn't raising validation errors and was causing numeric menu selection via inout to fail. This was fixes by adding strip() to inputs and additional isnumeric() calidation was added to validation functions. |
